...
def request(handler,*args):
    #check ~/pond/.user.auth and verify its content
    dprint('step into request_check_user_info')
    if not os.getenv('HOME_POND'): raise Exception("Evawiz environment does not set properly. Please source $POND_ROOT/pond.source or 'pond.source' file where the Evawiz installed.")
    #check local user state 
    info = handler.get_auth_info()
    if not info:
        print('You are not connected to pond server. Please try connect or register first.')
        return None
    ...
    if check_result == 'user connect info not right':
        print("""Your connect information is outdated. Please reconnect to pond server.""")
        return None

    print("""You are not connected to server. Please connect to a pond account.
    If you do not have a pond account, please use "eva register" to register one first""")
    return None 

def request_check_only(handler,*args):
    #check ~/pond/.user.auth and verify its content
    dprint('step into request_check_user_info')
    if not os.getenv('HOME_POND'):
        raise Exception("Evawiz environment does not set properly. Please source $EVAIZ_ROOT/pond.source or 'pond.source' file where the Evawiz installed.")
    #check local user state 
    info = handler.get_auth_info()
    if not info:
