...
def request(handler,*args):
    ...
    template_dir=os.getenv('POND')+'/Template'
    pond_dir = os.getenv('HOME_POND')
    module_dir = pond_dir +'/'+module_name
    if from_module:
        from_module_dir = pond_dir +'/'+from_module
    ...
    #try to new module from local pond by nfs
    #cp the Template dir to POND DIR
    if ( lang_type != 'c++' ): template_dir+='.'+lang_type
    if not os.path.exists(template_dir):
        print('host pond setting is not ready. Please contact your admin.')
        return
    #check pond dir
    ...
    if from_module:
        ...
        shell_status_output("%s/bin/change_from_to %s %s %s"%
                                 (pond_root,module_dir,from_module,module_name) )
        ...
        shell_status_output('cp %s/.evarunfile %s/'%(from_module_dir,module_dir) )
        if lang_type == 'cu':
            shell_status_output("%s/bin/rename -f 's/.cpp$/.cu/g' %s/*"%(pond_root,module_dir))
        dprint('try rename');
        shell_status_output("%s/bin/change_from_to %s %s %s"%
                                 (pond_root,module_dir,'Template',module_name) )
        dprint('local operation finished');
    #rename
    ...
    if result == 'server ready':
        ...
        (status,output)=shell_status_output( "cd %s; %s/bin/trash %s"%(pond_root,pond_dir,module_name) )
