...
#def request_connect_user(self):
    ...
    if reply_info != 'allow to connect':
        print('You are not permited to connect to pond server. Please contact the admin of your host')
        handler.send_eof()

    #get pond_account and its password to connect
    #username
    sys.stdout.write("pond account username >")
    sys.stdout.flush()
    while True:
        user_name = input()
        user_name.strip()
    ...
    if ans != 'failed to connect':
        ...
        print('succeed to connect pond server.')
