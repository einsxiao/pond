...
#def request_load_program(self,program_name = '', mode = 'silent'):
    ...
    #writable to $(POND_ROOT)/programs
    if os.access( os.getenv("POND_ROOT")+"/programs", os.R_OK ):
        if ans == 'program not exist':
            ...
            return None
        #try load
        cmd = os.getenv('POND_ROOT') + "/bin/update-eva "+program_name;
    ...
    #check if pondd is running
    if os.path.exists( os.getenv('POND_ROOT')+"/pids/pond_host_server.pid" ):
        ...
        request_file = os.getenv("POND_ROOT")+"/programs.request"
        file_content_append( request_file, program_name )
        return
    else:
        print("You have no access to the pond directory and the host server is neither running. Please contact your admin to deal with this." )
        return
    pass
