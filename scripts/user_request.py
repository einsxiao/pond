...
class UserRequest:
    ...
    conn = None
    connection = None
    server_addr = os.getenv("POND_SERVER") or 'server.powerbyte.online'
    server_port = os.getenv("POND_SERVER_PORT") or "5555"
    proxy = None
    aes = None
    closed = False

    def __init__(self, host='server.powerbyte.online', port=5555, proxy=None ):
        ...
        if self.proxy:
            ...
            if len( self.proxy ) != 2:
                raise Exception("POND_PROXY is not specified in the right form.");
            pass
        pass
    ...
    def contact_server(self,operation='say_hello',response_type='encrypted'):
        ...
        dprint("send hello")
        self.send("Hello POND! You are great!")
    ...
    def clear_auth_info(self):
        pond_dir= os.getenv('POND_HOME')
        if not os.path.exists(pond_dir): return
        auth_file = pond_dir + '/.user.auth'
        if os.path.exists(auth_file): os.system("chmod 600 "+auth_file)
    ...
    def get_settings(self,setting_name='eva'):
        pond_dir= os.getenv('POND_HOME')
        if not os.path.exists(pond_dir): return None
        setting_file = pond_dir+item+'.conf'
