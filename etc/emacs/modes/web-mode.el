...
(defcustom web-mode-script-padding 0
...
(defcustom web-mode-markup-indent-offset
  (if (and (boundp 'standard-indent) standard-indent) standard-indent 1)
...
(defcustom web-mode-sql-indent-offset 1
...
(defcustom web-mode-indent-style 1
  "Indentation style."
  :group 'web-mode
  :type '(choice (const :tag "default (all lines are indented)" 1)
  ...
  (let ((open-ctx (web-mode-bracket-up pos language limit)) offset)
    ...
    (cond
     ...
     ((and (member language '("javascript" "jsx" "ejs"))
      ...
      (setq offset (+ (plist-get open-ctx :indentation) (* language-offset 1))))
     (t
      (setq offset (+ (plist-get open-ctx :indentation) language-offset)))
     ) ;cond
    (cons (if (< offset initial-column) initial-column offset) open-ctx)
    ...
    (let* ((ctx (web-mode-bracket-up pos language limit))
      ...
      (cond
       ...
       ((and (member language '("php"))
             ...
             (not (looking-at-p "case\\|default")))
        (setq indentation (+ indentation (* language-offset 1)))
        )
       ((and (member language '("php"))
             (eq char ?\{)
             (goto-char pos)
    ...
    (when h
      ...
      (cond
       ...
       ((string-match-p "\\<\\(end select\\)" line)
        (setq out (- prev-indentation (* 1 language-offset))))
  ...
  (let (beg end pos tag sep)
    (save-excursion
      ...
      (when (and beg end (> end 0))
        ...
        (when (string= sep "\n") (indent-region beg (+ end (* (+ 3 (length tag)) 1))))
        )
      ) ;save-excursion
    (web-mode-go beg)))
