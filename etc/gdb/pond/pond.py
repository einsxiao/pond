...
class ObjectPrinter(object):
    ...
    def to_string(self):
        if self.val:
            #self._str = str( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->ToString()' ) );
            #print(self._objid)
            self._type = str( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->type()' ) );
            self._cnt = str( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->cnt()' ) );

            if self._type == "pond::EvaType::Number":
                self._val = str( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->Number()' ) );
                return "Number({0})={1}".format(self._cnt, self._val )

            if self._type == "pond::EvaType::String":
                self._val = str( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->Key()' ) );
                return "String({0})={1}".format(self._cnt, self._val )

            if self._type == "pond::EvaType::Symbol":
                self._val = str(gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->Key()' ) );
                return "Symbol({0})={1}".format(self._cnt, self._val )

            if self._type == "pond::EvaType::List":
                self._size = gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->ElementsSize()' );
                self._idx_second = int( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->idx().second' ) );
                self._idx_first = int( gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->idx().first' ) );
        ...
        #return  gdb.parse_and_eval( '((pond::Object*)'+str( self.val.address )+')->ToString()' )['_M_dataplus']['_M_p'].string();
        return "pond::NullObject"

    def children(self):
        if self._type != "pond::EvaType::List": return ''
