...
class EvaObjPrinter:
    ...
    def to_string(self,count=0):
        ...
        if ( self.rtype == "pond::NumberT" ):
            return str(self.rre)

        if ( self.rtype == "pond::ComplexT" ):
            return "("+str( self.rre )+","+str( self.rim)+")"

        if ( self.rtype == "pond::StringT" ):
            if ( self.rre ): self.rkey=self.rv1['key'].string();
            return '"'+self.rkey+'"'

        if ( self.rtype == "pond::SymbolT" ):
            if ( self.rre ): self.rkey=self.rv1['key'].string();
            res = ""
...
def build_pretty_printers():
    ...
    pp.add_printer('EvaObj',"^pond::EvaObj$",EvaObjPrinter)
    return pp
