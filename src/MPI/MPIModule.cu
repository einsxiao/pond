...
MPIModule::MPIModule():Module(MODULE_NAME)
{
}
    ...
    if ( rankID == 0 ){                                                 \
      ...
      for (int i=rankSize-1; i>= 0; i-- ){                              \
        ...
        while ( true ){                                                 \
          index_copy(dim[0], gind, lind);                               \
          gind[ith-1] += dim[ ith ] * i;                                \
          local_matrix.Part(lind) = global_matrix.Part( gind );         \
          if (not index_incr(dim, lind ) ) break;                       \
        }                                                               \
        if ( i != 0 ){                                                  \
          MPI_Send( dim, 1 , MPI_INT,i,1,MPI_COMM_WORLD);               \
    ...
    if ( rankID == 0 ){                                                 \
      ...
      for ( int i=0; i< rankSize; i++ ){                                \
        ...
        while ( true ){                                                 \
          index_copy(dim[0], gind, lind);                               \
          gind[ith-1] += dim[ ith ] * i;                                \
          global_matrix.Part( gind ) = local_matrix.Part( lind );       \
          if ( not index_incr(dim, lind ) ) break;                      \
        }                                                               \
      }                                                                 \
      delete []dim; delete []lind; delete []gind;                       \
...
int MPIModule::PD_MPITest(Object&Argv)
{
