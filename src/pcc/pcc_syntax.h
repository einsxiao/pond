#ifndef __PDCC_SYNTAX_H__
#define __PDCC_SYNTAX_H__
...
public:
  ...
  string     ToString(){return pond::ToString(row)+":"+pond::ToString(column);}
};
...
public:
  ...
  struct __node_rec{
    ...
    u_int   pos, level_pos;
    Node   *self;
    __node_rec(){ topNode=NULL; parent=NULL; };
    __node_rec(Node*t,Node*p,u_int _p,u_int _lp, Node*_self){
      topNode = t; parent = p; pos = _p; level_pos = _lp; self = _self;
    }
  };
  //////////////////////////////////////////////////
  vector< __node_rec >        pragmas;
  vector< __node_rec >        pd_functions;
  __node_rec                  pd_constructor;
  __node_rec                  pd_deconstructor;
  //////////////////////////////////////////////////

  vector< pair<int,int> >     lineInfo;
  vector< __line_info >       source;
  vector< vector<int> >       old_2_new; //old row number -> new row number, in source vector
  string                      outContent;
...
public:
  ...
  void       deal_launch_kernel(__node_rec&,int serailN);
  void       deal_pd_module();
  void       deal_pd_functions();
  void       rewrite();
  void       phrasing();
  void       output();
