...
namespace pond{
  ...
  // in x64 size_t = u_long
  // dimension 1, 2, 3 can be no more that 2^32(42,9496,7296)
  // dimension 4, 5, 6, 7, 8 can be no more than 2^16(6,5536)
  template<class type>
  class Matrix_T{
  public:
    ...
    // size of matrix 1+1+5*2+1*4+4*8 = 40 B
  public:
    __cond_host_device__           Matrix_T();
    __cond_host_device__          ~Matrix_T();
    __cond_host_device__           Matrix_T(const Matrix_T&);
    Matrix_T(u_int*dim, int dataPosition=MatrixHost);
    Matrix_T(u_int n,...);
    void                           SetDim(const u_int p,const u_int Np);
    __cond_host_device__ u_int     DimN()const;
    __cond_host_device__ u_int     Dim(const u_int p)const;
    size_t                         Size()const;
    u_int                         *NewDimArray();
    void                           Init(u_int*dim,int dataPosition=MatrixHost,bool veryInit=false);
    ...
    __cond_host_device__ type     &Part(u_int*pArr);
    __cond_host_device__ type     &Index(u_int*pArr);
    __cond_host_device__ type     &operator[](size_t p);
    __cond_host_device__ type     &operator()(u_int p);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3,u_int p4);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7);
    __cond_host_device__ type     &operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8);
    __cond_host_device__ type     &at(u_int p);
    __cond_host_device__ type     &at(u_int p1,u_int p2);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3,u_int p4);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7);
    __cond_host_device__ type     &at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8);

    //fortran index style with collomn major and index starting with 1
    __cond_host_device__ type     &fPart(u_int *pArr);
    __cond_host_device__ type     &fIndex(u_int *pArr);

    Matrix_T                       SubMatrix(u_int n,...); // n is the number of suffix of to draw the subMatrix out
    Matrix_T                       ReShape(u_int n,...); //same Size in different shape, Data kept
    Matrix_T                       ReShape(u_int*dim); //same Size in different shape, Data kept
    ...
    static size_t CalculateSize(u_int*dim);
  };
    
  template<class type>
    size_t Matrix_T<type>::CalculateSize(u_int*dim){
    size_t size = 1;
...
#undef CP
  ...
  template<class type> __cond_host_device__ u_int Matrix_T<type>::Dim(const u_int p)const{
    switch (p){
    case 0: return DimN();
    case 1: return D1;
    case 2: return D2;
  ...
  template<class type> void Matrix_T<type>::SetDim(const u_int p,const u_int Np){
    switch (p){
    case 0: return ;
    case 1: D1 = Np; return;
    case 2: D2 = Np; return;
  ...
  template<class type> __cond_host_device__ u_int Matrix_T<type>::DimN()const{
    return ND;
  }

  template<class type> u_int *Matrix_T<type>::NewDimArray(){
    u_int *dim = new u_int[ ND +1 ];
    dim[0] = ND;
    for ( u_int i=1;i<=ND;i++)
      dim[i] = Dim(i);
    return dim;
  }
  template<class type> size_t Matrix_T<type>::Size()const{
    size_t size = D1;
    for ( u_int i=2; i<= ND ; i++ )
      size *= Dim(i);
    return size;
  }
...
#define _check_dimN(N) if ( ND != N ){                                  \
    ThrowError("Matrix::at","Try access element of Matrix with "+       \
               ToString((u_int)ND)+" dimension(s) with "+ToString((u_int)N)+" index(s)."); \
  }
#define _check_dimith(N) if ( p##N >= D##N ){                   \
	ThrowError("Matrix::at","index "+ToString((u_int)p##N)+" of dimension "+ToString((u_int)N)+" out of range 0 .. "+ToString((u_int)(D##N-1))+"."); \
}
#define _check_fdimith(N) if ( p##N > D##N ){                   \
	ThrowError("Matrix::at","index "+ToString((u_int)p##N)+" of dimension "+ToString((u_int)N)+" out of range 1 .. "+ToString((u_int)D##N)+"."); \
}
  //1
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1){
#ifdef __CUDA_ARCH__
    return DataDevice[ p1 ];
#else
    return Data[ p1 ];
  ...
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1){
#ifdef __CUDA_ARCH__
    return DataDevice[ p1 ];
#else
    _check_dimN(1);
  ...
  //2
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2){
#ifdef __CUDA_ARCH__
    return DataDevice[ p1*D2+p2 ];
#else
    return Data[ (size_t)p1*D2+p2 ];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2){
#ifdef __CUDA_ARCH__
    return DataDevice[ p1*D2+p2 ];
#else
    _check_dimN(2);
  ...
  //3
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3){
#ifdef __CUDA_ARCH__
    return DataDevice[(p1*D2+p2)*D3+p3 ];
#else
    return Data[((size_t)p1*D2+p2)*D3+p3 ];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3){
#ifdef __CUDA_ARCH__
    return DataDevice[((size_t)p1*D2+p2)*D3+p3 ];
#else
    _check_dimN(3);
    _check_dimith(1);
    _check_dimith(2);
  ...
  //4
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3,u_int p4){
#ifdef __CUDA_ARCH__
    return DataDevice[ ((p1*D2+p2)*D3+p3)*D4+p4 ];
#else
    return Data[ (((size_t)p1*D2+p2)*D3+p3)*D4+p4 ];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3,u_int p4){
#ifdef __CUDA_ARCH__
    return DataDevice[ (((size_t)p1*D2+p2)*D3+p3)*D4+p4 ];
#else
    _check_dimN(4);
    _check_dimith(1);
    _check_dimith(2);
  ...
  //5
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5){
#ifdef __CUDA_ARCH__
    return DataDevice[(((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5 ];
#else
    return Data[((((size_t)p1*D2+p2)*D3+p3)*D4+p4)*D5+p5 ];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5){
#ifdef __CUDA_ARCH__
    return DataDevice[(((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5 ];
#else
    _check_dimN(5);
  ...
  //6
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6){
#ifdef __CUDA_ARCH__
    return DataDevice[((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6];
#else
    return Data[(((((size_t)p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6){
#ifdef __CUDA_ARCH__
    return DataDevice[((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6];
#else
    _check_dimN(6);
  ...
  //7
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7){
#ifdef __CUDA_ARCH__
    return DataDevice[(((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7];
#else
    return Data[((((((size_t)p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7){
#ifdef __CUDA_ARCH__
    return DataDevice[(((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7];
#else
    _check_dimN(7);
  ...
  //8
  template<class type> __cond_host_device__ type&Matrix_T<type>::operator()(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8){
#ifdef __CUDA_ARCH__
    return DataDevice[((((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7)*D8+p8];
#else
    return Data[(((((((size_t)p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7)*D8+p8];
#endif
  };
  template<class type> __cond_host_device__ type&Matrix_T<type>::at(u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8){
#ifdef __CUDA_ARCH__
    return DataDevice[((((((p1*D2+p2)*D3+p3)*D4+p4)*D5+p5)*D6+p6)*D7+p7)*D8+p8];
#else
    _check_dimN(8);
...
#undef _check_fdimith
  ...
  template<class type> __cond_host_device__ type&Matrix_T<type>::Part(u_int *pArr){
#ifndef __CUDA_ARCH__
    u_int index=pArr[0];
#else
    size_t index=pArr[0];
  ...
  template<class type> __cond_host_device__ type&Matrix_T<type>::Index(u_int *pArr)
  {
    return Part(pArr);
  }

  template<class type> void Matrix_T<type>::Init(int *dimArr,int dataPosition,bool veryInit){
    Init((u_int*)dimArr, dataPosition, veryInit);
  }
  template<class type> void Matrix_T<type>::Init(u_int *dimArr,int dataPosition,bool veryInit){
...
#  endif
  ...
  template<class type> void Matrix_T<type>::Init(u_int n,...) {
    va_list args;
    va_start(args,n);
    u_int *arr	=	new u_int[ n+ 1 ];
    arr[0]=n;
    for (int i=1; i<=n; i++)
      arr[i]	=	va_arg(args, u_int);
  ...
  template<class type> Matrix_T<type>::Matrix_T(u_int n,...) {
    //Matrix_T x(2,8,8);
    va_list args;
    va_start(args,n);
    u_int *arr	=	new u_int[ n+ 1 ];
  ...
  template<class type> Matrix_T<type> Matrix_T<type>::SubMatrix(u_int n, ...){
    if ( n >= ND ) {
  ...
  template<class type> Matrix_T<type> Matrix_T<type>::ReShape(u_int n, ...)
  {
    va_list args;
    va_start(args,n);
    u_int *new_dim = new u_int(n+1);
    size_t eleSize;
  ...
  template<class type> Matrix_T<type> Matrix_T<type>::ReShape(u_int *dim)
  {
    size_t size = Matrix_T<type>::CalculateSize(dim);
    if ( size > Size() ) {
      _Erroring("Matrix","Matrix can only be reshaped to a matrix of the same or smaller size."); 
      return (*this); 
    }
...
#undef OPER_MATRIX_OVERLOAD_CU
...
#ifdef __CUDACC__ 
  template<class type>
  __global__ void Matrix_Min_Kernel(type *arr, u_int n_in)
  {
    u_int tid= blockIdx.x*blockDim.x+threadIdx.x;
    u_int n_cal= gridDim.x*blockDim.x;
    u_int n_out= n_in/2;
    u_int n_inc; if (n_in%2==0) n_inc=n_out; else n_inc=n_out+1;
    while (tid<n_out){
  ...
  template<class type>
  __global__ void Matrix_Max_Kernel(type *arr,u_int n_in)
  {
    u_int tid= blockIdx.x*blockDim.x+threadIdx.x;
    u_int n_cal= gridDim.x*blockDim.x;
    u_int n_out= n_in/2;
  ...
  template<class type>
  __global__ void Matrix_Total_Kernel(type *arr,u_int n_in)
  {
    u_int tid= blockIdx.x*blockDim.x+threadIdx.x;
    u_int n_cal= gridDim.x*blockDim.x;
    u_int n_out= n_in/2;
...
#pragma omp parallel for reduction(+:sum)
  ...
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1){
    return a[ p1 ];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2){
    return a[ p1*d[2]+p2 ];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3){
    return a[(p1*d[2]+p2)*d[3]+p3 ];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4){
    return a[ ((p1*d[2]+p2)*d[3]+p3)*d[4]+p4 ];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4,u_int p5){
    return a[(((p1*d[2]+p2)*d[3]+p3)*d[4]+p4)*d[5]+p5 ];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6){
    return a[((((p1*d[2]+p2)*d[3]+p3)*d[4]+p4)*d[5]+p5)*d[6]+p6];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7){
    return a[(((((p1*d[2]+p2)*d[3]+p3)*d[4]+p4)*d[5]+p5)*d[6]+p6)*d[7]+p7];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8){
    return a[((((((p1*d[2]+p2)*d[3]+p3)*d[4]+p4)*d[5]+p5)*d[6]+p6)*d[7]+p7)*d[8]+p8];
  };
  template<class type> __cond_host_device__ type&MatrixPart(type*a,u_int*d,u_int p1,u_int p2,u_int p3,u_int p4,u_int p5,u_int p6,u_int p7,u_int p8,u_int p9){
    return a[(((((((p1*d[2]+p2)*d[3]+p3)*d[4]+p4)*d[5]+p5)*d[6]+p6)*d[7]+p7)*d[8]+p8)*d[9]+p9];
  };
