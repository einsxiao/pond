...
Runmod::Runmod()
{
  ...
  moduleName = "";
  if ( EvaKernel->argc > 1 ){
    if ( strcmp( EvaKernel->argv[0],"pond" ) == 0 ){
      if ( EvaKernel->argc > 2 ){
...
void Runmod::loadModuleDefault()
{
  ...
  defaultInputFileName = GetEnv("HOME_POND")+"/"+moduleName+"/input.ev";
...
void Runmod::runLocal()
{
  ...
  proc->start("/usr/bin/env", QStringList()<<"pond"<<QString( inputFileName.c_str() ) );
...
void Runmod::runPbs()
{
  ...
  ofs<<"#PBS -N pond_Runmod_"<< moduleName<<"\n";
  ...
  ofs<<"pond "<< inputFileName <<" > pbs.outout\n"; 
  ofs.close();
